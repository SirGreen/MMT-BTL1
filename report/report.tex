...
\subsection{Chức năng ứng dụng chia sẻ tệp BitTorrent và giao thức giao tiếp}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{HTTP:} Để tải xuống các tệp .torrent hoặc truy cập các liên kết nam châm.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent Tracker (HTTP):} Cách tiếp cận tập trung để tìm kiếm peer.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent qua TCP/UDP:} Quản lý việc trao đổi các mảnh của tập tin.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent qua TCP/UDP:} Quản lý việc gửi các mảnh tới các peer.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent qua TCP/UDP:} Kiểm soát việc tiếp nhận các mảnh từ nhiều peer cùng lúc.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Xác minh Internal Hash (SHA-1):} Được thực hiện trong ứng dụng để xác nhận tính chính xác của dữ liệu đã nhận.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent:} Sử dụng cơ chế tích hợp để quản lý tỷ lệ tải lên/xuống và ưu tiên các peer tương tác.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Trao đổi khóa Diffie-Hellman:} Establishes a shared secret between peers to encrypt communication.
        \item \textbf{AES (Advanced Encryption Standard, Tiêu chuẩn mã hóa nâng cao):} Mã hóa dữ liệu truyền đi thực tế để đảm bảo an toàn.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent:} Bao gồm các cơ chế để thử lại các lần tải xuống không thành công và phát hiện lỗi.
    \end{itemize}
...
\subsection{Chức năng dành riêng cho Tracker}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent Tracker (HTTP):} Được sử dụng để đăng ký các peer với một tracker tập trung.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent Tracker (HTTP):} Truyền đạt trạng thái của peer tới trình theo dõi.
    \end{itemize}
    ...
    \textit{Giao thức giao tiếp:}
    \begin{itemize}
        \item \textbf{Giao thức BitTorrent Tracker (HTTP):} Cung cấp danh sách các peer đang hoạt động từ tracker.
    \end{itemize}
...
% \includesvg[width=0.5\textwidth]{MVC.svg}

% \includesvg[width=0.5\textwidth]{join_module.svg} 

% \includesvg[width=0.5\textwidth]{have_module.svg} 

% \includesvg[width=0.5\textwidth]{file_sharing_module.svg}

% \includesvg[width=0.5\textwidth]{exit_module.svg} 
